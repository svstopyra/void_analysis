...
def test_trim_los_list_basic(synthetic_los_data):
    los_pos, spar_bins, sperp_bins, radii = synthetic_los_data
    los_trimmed, voids_used = trim_los_list(los_pos, spar_bins, sperp_bins, radii)
...
def test_get_trimmed_los_list_per_void_basic(synthetic_los_data):
    los_pos, spar_bins, sperp_bins, radii = synthetic_los_data
    trimmed = get_trimmed_los_list_per_void(los_pos, spar_bins, sperp_bins, radii)
...
def test_get_field_from_los_data_basic(synthetic_los_data):
    los_pos, spar_bins, sperp_bins, radii = synthetic_los_data
    stacked = get_2d_void_stack_from_los_pos(los_pos, spar_bins, sperp_bins, radii)
    void_count = 5
    v_weight = np.ones(len(stacked))
    field = get_field_from_los_data(stacked, spar_bins, sperp_bins, v_weight, void_count)
...
def test_get_1d_real_space_field_regression(synthetic_los_data):
    ...
    ref_mean = np.load(os.path.join(SNAPSHOT_DIR, "real_space_profile_mean_ref.npy"))
    ref_std = np.load(os.path.join(SNAPSHOT_DIR, "real_space_profile_std_ref.npy"))
