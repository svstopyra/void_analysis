...
def mock_transform_inputs():
    ...
    return r_par, r_perp, z, Om, f, Delta

# ---------------------- UNIT TESTS ----------------------

def test_to_z_space_runs(mock_transform_inputs):
    r_par, r_perp, z, Om, f, Delta = mock_transform_inputs
...
def test_iterative_inverse_scalar_accuracy():
    ...
    s_par, s_perp = to_z_space(r_par, r_perp, z, Om, Delta=Delta, f=f)
    # Scalar inverse
    r_par_inv = iterative_zspace_inverse_scalar(s_par, s_perp, f, Delta)
    # Validate
...
def test_iterative_inverse_approx(mock_transform_inputs):
    r_par, r_perp, z, Om, f, Delta = mock_transform_inputs
    s_par, s_perp = to_z_space(r_par, r_perp, z, Om, Delta=Delta, f=f)
    r_par_inv = iterative_zspace_inverse(s_par, s_perp, f, Delta)
...
def test_to_real_space_matches_inverse(mock_transform_inputs):
    ...
    r_par_inv, r_perp_inv = to_real_space(
        s_par, s_perp, z, Om, Delta=Delta, f=f
    )
    assert np.allclose(r_par_inv, r_par, atol=1e-2)
