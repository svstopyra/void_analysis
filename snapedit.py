...
def gridTest(s,grid,lfl_corner=[0,0,0]):
    ...
    if(n - N != 0.0):
        ...
        if(
            ...
            # Nonstandard order:
            print("Warning - grid appears cubic, but non-standard " + 
                  "particle order detected. Exact centroids cannot be " + 
                  "computed.")
            return -1
        else:
...
def wrap(pos,boxsize,old_method=False):
    if old_method:
        ...
        wrap[np.where(wrap >= boxsize)] -= boxsize # To deal with rounding 
        # errors
    return wrap
...
# pos - array containing the positions we want to assign to grid points 
# (can be a simarray)
...
# lfl_corner - lower front left hand corner of the box (assumed [0,0,0] by 
# default)
def index(pos,boxsize,N,lfl_corner=[0,0,0]):
    ind = np.zeros(pos.shape,dtype='int')
    # Grid centroids are actually offset by half a cell from the lower front 
    # left corner, so we have to subtract off this part before rounding to the 
    # closest grid.
    ind[:] = (np.round((pos - lfl_corner)*N/boxsize - 0.5)).astype(int)
    return ind

def grid_offset(pos,boxsize,N,lfl=[0,0,0]):
    ind = index(pos,boxsize,N,lfl) # Grid indices for each point 
    # (assuming uniform initial grid)
    grid = (ind*boxsize/N) + (boxsize/(2*N)) # positions of centroids for 
    # each cell.
    return pos - grid

# Gets the zeldovich approximation factor between position and velocity offsets:
def zeldovich_factor(s,factor=1):
    return factor*100.0*np.sqrt(
        s.properties['omegaM0']/(s.properties['a']**3)
        + s.properties['omegaL0'])*np.sqrt(s.properties['a']
    )
...
def zeldovich_test(s,factor=1,tol=1e-4,lfl=[0,0,0],grid = None):
    ...
    # Get positions of particles subtracting off what the offsets would be if 
    # we were using the zeldovich approximation:
    if grid is None:
    ...
    if(perm != -1):
        ...
        print("Average difference from expected grid centroids as a " + 
              "fraction of cell size after inverting Zeldovich " + 
              "approximation is " + str(mean))
        ...
        print("Average displacement from grid centroids as a fraction of" + 
              " cell size is: " + str(mean_disp))
        ...
        if (np.abs(mean) < tol):
            ...
            if(len(violaters[0]) > 0):
                print(
                      str(len(violaters[0])) + " particles not sufficiently " + 
                      "close to grid points after inversion. Maximum " + 
                      "violation is " + str(np.max(diff_ratio[violaters[0]]))
                )
            return True
        else:
            print("ICs do not appear to be Zeldovich.")
            if(len(violaters[0]) > 0):
                print(str(len(violaters[0])) + " particles not sufficiently " + 
                "close to grid points after inversion. Maximum violation is " +
                str(np.max(diff_ratio[violaters[0]]))
            )
            return False
    else:
        print("ICs are not cubic - cannot determine grid centroids to " + 
              "assess whether or not the Zeldovich approximation is used.")
        return False




# Computes the logarithmic derivative of the linear structure growth function,
#  used to get peculiar velocities in the Zeldovich approximation:
def f1(Om,Ol,z):
...
# Construct a list of indices that we expect for an NxNxN grid using 
# lin(i,j,k) = N^2*i + N*j + k as the linear index
def gridList(N):
    return gridListPermutation(N)

# Inverted list of indices, where we use lin(i,j,k) = N^2*k + N*j + i as the
# linear index
...
def centroid(s,N,units=None,perm=(0,1,2)):
    ...
    return (
        grid*s.properties['boxsize']/N + s.properties['boxsize']/(2*N)
    ).in_units(units)
...
def reverseICs(
        s,filename=None,units=None,fmt=None,inverted=False,factor=1,perm=None
    ):
    if(units == None):
        # Inherit the units from the previous snapshot
        units = s['pos'].units
    ...
    if(perm == None):
        # Try to figure out what sort of particle order the file is using, 
        # unless this has been specified using perm
    ...
    if(perm != -1):
        ...
        # If perm == -1, then we probably have a zoom-grid or other non-cubic 
        # grid. We will have to approximate the correct grid centroids 
        # assuming the Zeldovich approximation holds.
        # getGridFromZeldovich gives us something in Mpc, so convert back to 
        # the original units to ensure consistency:
...
def zones2Particles(partfile):
    ...
    for k in range(0,zoneCount):
        zoneParts.append(
            particle_data[iterator:(iterator+particle_data[iterator-1])]
        )
...
class ZobovVoid:
    def getParticles(self,zoneParts,voidZones):
        ...
        for k in range(1,len(voidZones[self.id])):
            partList = np.concatenate(
                (partList,zoneParts[voidZones[self.id][k]])
            )
        if(len(partList) != self.npart):
            raise(
                Exception("Particle list does not match expected " + 
                          "number of particles.")
            )
        return partList
...
def importVoidList(voidData,zoneData,partData):
    ...
    for k in range(0,len(voidProperties)):
        voidList.append(
            ZobovVoid(
                sortedList[k],voidProperties[sortedList,:],zoneParts,voidZones
            )
        )
    return voidList
...
def getParticlesInHalos(snap,halos,pynbody_offset=0):
    ...
    counter = 0
    for k in range(0,len(halos)):
        in_halos[counter:(counter + lengths[k])] = halos[
            k+pynbody_offset
        ]['iord']
    ...
    # Have to sort the indices of snap, because these are usually not in 
    # ascending order:

    return snap[np.argsort(snap['iord'])][np.unique(in_halos)]

# Outputs a copy of a snapshot filt, with the particles re-ordered to be in 
...
def newSnap(
        newFilename,dmPos,dmVel,dmMass = None,gasPos=None,gasVel=None,
        gasMass=None,properties=None,fmt=None,indices=None,gasIndices=None
    ):
    includeGas = False
    if gasPos is None:
    ...
    if includeGas:
        snew.gas['pos'] = gasPos
        if gasVel is None:
            gasVel = pynbody.array.SimArray(
                np.zeros(gasPos.shape),"km a**1/2 s**-1"
            )
        snew.gas['vel'] = gasVel
        if (dmMass is not None):
            if gasMass is None:
                gasMass = pynbody.array.SimArray(
                    np.zeros(len(gasPos)),"1e+10 Msol h**-1"
                )
...
# Pads the points from a snapshot periodically, adding duplicates of 
# particles from one end of the box to the other.
def periodicPadding(snapshot,width,boxsize):
    ...
    points_new = np.vstack(
        (points,points[plowx] + np.array([boxsize,0,0]),
        points[puppx] - np.array([boxsize,0,0]))
    )
    indices = np.hstack((indices,indices[plowx[0]],indices[puppx[0]]))
    plowx = np.where((points_new[:,1] > 0.0) & (points_new[:,1] <= width))
    puppx = np.where(
        (points_new[:,1] > boxsize - width) & (points_new[:,1] <= boxsize)
    )
    points_new = np.vstack(
        (points_new,points_new[plowx] + np.array([0,boxsize,0]),
        points_new[puppx] - np.array([0,boxsize,0]))
    )
    indices = np.hstack((indices,indices[plowx[0]],indices[puppx[0]]))
    plowx = np.where((points_new[:,2] > 0.0) & (points_new[:,2] <= width))
    puppx = np.where(
        (points_new[:,2] > boxsize - width) & (points_new[:,2] <= boxsize)
    )
    points_new = np.vstack(
        (points_new,points_new[plowx] + np.array([0,0,boxsize]),
        points_new[puppx] - np.array([0,0,boxsize]))
    )
    indices = np.hstack((indices,indices[plowx[0]],indices[puppx[0]]))
...
def getDisplacements(
        ics,z = 0,perm=(0,1,2),factor = 1,lfl_corner=[0,0,0],centroids = None
    ):
    if centroids is None:
        # Attempt to auto-determine the grid.
        if perm == "auto":
            perm = gridTest(
                ics,getGridFromZeldovich(ics,factor),lfl_corner=[0,0,0]
            )
            if perm == -1:
                raise Exception("Could not determine grid order. " + 
                                "Likely grid is non-cubic or has " + 
                                "non-standard order.")
        N = np.round(np.cbrt(len(ics))).astype(int)
        Nex = np.cbrt(len(ics))
        if (N - Nex) != 0:
            raise Exception("Grid is not cubic - could not " + 
                            "auto-determine centroids.")
...
def extrapolateZeldovich(
        ics,z = 0,perm=(0,1,2),factor = 1,lfl_corner=[0,0,0],
        centroids = None,filename = None
    ):
    if centroids is None:
        # Attempt to auto-determine the grid.
        if perm == "auto":
    ...
    zic = 1.0/ics.properties['a'] - 1
    Omegam = ics.properties['omegaM0']
    velz = ics['vel']*cosmology.linearGrowthf(
        z,Omegam
    )/cosmology.linearGrowthf(zic,Omegam)
    if filename == "auto":
        filename = ics._filename + "_zeldovich_z" + str(z)
    newProperties = copy.deepcopy(ics.properties)
    newProperties['a'] = 1.0/(z + 1.0)
    snew = newSnap(
        filename,posz,velz,dmMass = ics['mass'],fmt = type(ics),
        properties = newProperties,indices= ics.dm['iord']
    )
    return snew
